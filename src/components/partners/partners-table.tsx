...
import { Pencil, Ban, BookOpen, Trash2 } from "lucide-react";
import { bulkDeletePartners, deactivatePartner, deletePartner } from "@/app/actions/partners";
...
export default function PartnersTable({ partners }: { partners: PartnerRow[] }) {
    ...
    const handleBulkDelete = async () => {
        ...
        if (!count) return;
        if (!confirm(`Permanently delete ${count} selected partner(s)? This cannot be undone.`)) {
            return;
        }

        const res = await bulkDeletePartners([...selectedIds]);
        if (res.success) {
            setSelectedIds(new Set());
            router.refresh();
        } else {
            alert(res.error || "Failed to delete partners");
        }
    };
            ...
            {selectedIds.size > 0 && (
                <div className="flex flex-wrap items-center gap-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3">
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-600 text-white text-sm font-bold hover:bg-red-700"
                    >
                        <Trash2 size={14} />
                        Delete Selected
                    </button>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {partners.map((partner) => (
                            <tr key={partner.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-4 text-slate-600">{partner.bankName}</td>
                                <td className="px-4 py-4 font-mono text-slate-700">
                                    {partner.accountNumber}
                                </td>
                                <td className="px-4 py-4 text-slate-500">{partner.vatNo || "—"}</td>
                                <td className="px-4 py-4">
                                    <span
                                        className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase ${
                                            partner.isActive
                                ...
                                <td className="px-4 py-4 text-right">
                                    <div className="flex gap-2 justify-end">
                                        ...
                                        {partner.isActive && (
                                            <button
                                                onClick={() =>
                                                    handleDeactivate(partner.id, partner.name)
                                                }
