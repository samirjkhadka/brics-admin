...
export const sendFlightAlert = async (to: string, tx: Transaction, timeLeft: string) => {
    const passengerLabel = formatPassengerNames(tx.passengerNames) || "Unknown";

    const mailOptions = {
        from: `"BRICS Notifications" <${process.env.EMAIL_USER}>`,
        to,
        subject: `Flight Alert: ${passengerLabel} - ${timeLeft} to Travel`,
        html: `
